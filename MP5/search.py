...
def best_first_search(starting_state):
    ...
    while frontier:
        ...
        if currentState.is_goal():
            return backtrack(visited_states, currentState)

        for state in currentState.get_neighbors():
            if state not in visited_states or state.dist_from_start < visited_states[state][1]:
                visited_states[state] = (currentState, state.dist_from_start)
...
def backtrack(visited_states, goal_state):
    ...
    while visited_states[currentState][0] is not None:
        currentState = visited_states[currentState][0]
