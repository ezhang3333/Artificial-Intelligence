...
def fit(train_set,train_labels,dev_set,epochs,batch_size=100):
    ...
    net = NeuralNet(lrate=0.01, loss_fn=loss_fn, in_size=in_size, out_size=out_size)

    losses = []
    for epoch in range(epochs):
        epoch_loss = 0.0
        for batch in train_loader:
            x_batch = batch['features'] if 'features' in batch else batch['x']
            y_batch = batch['labels'] if 'labels' in batch else batch['y']
